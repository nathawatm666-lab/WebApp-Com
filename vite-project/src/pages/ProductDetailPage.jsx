        ...
        return (
            <div className="max-w-7xl mx-auto px-4 py-10">
                <div className="animate-pulse grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="aspect-square bg-card rounded-xl" />
                    <div className="space-y-4">
                        ...
                        <div className="h-12 bg-card rounded w-1/2 mt-6" />
                    </div>
                </div>
            </div>
        );
    ...
    if (!product) {
        return (
            <div className="max-w-7xl mx-auto px-4 py-20 text-center">
                <p className="text-text-muted text-lg">ไม่พบสินค้า</p>
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 py-6">
            ...
            <nav className="flex items-center gap-2 text-sm text-text-muted mb-6 flex-wrap">
                ...
                <span>/</span>
                <span className="text-text-main truncate">{product.name}</span>
            </nav>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {/* Image Gallery */}
                <div>
                    <div className="bg-card rounded-xl border border-border overflow-hidden aspect-square flex items-center justify-center p-4 relative">
                        <img
                        ...
                        <div className="flex gap-2 mt-3">
                            {images.map((img, i) => (
                                <button
                                    ...
                                    className={`w-16 h-16 rounded-lg border overflow-hidden cursor-pointer ${i === selectedImage ? 'border-accent' : 'border-border'
                                        }`}
                                >
                ...
                <div>
                    ...
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <span className="text-text-muted text-sm">{product.rating} ({product.review_count} รีวิว)</span>
                        <span className="text-border">|</span>
                        <span className="text-text-muted text-sm">SKU: {product.sku}</span>
                    </div>

                    {/* Price */}
                    <div className="bg-primary border border-border rounded-xl p-4 mb-4">
                    ...
                    <div className="flex items-center gap-2 mb-6">
                        ...
                        {product.stock > 0 && product.stock < 10 && (
                            <span className="text-text-muted text-sm">เหลือ {product.stock} ชิ้น</span>
                        )}
                    </div>
                        ...
                        <div className="flex items-center gap-3 mb-4">
                            <div className="flex items-center border border-border rounded-lg">
                                <button
                                    ...
                                    className="px-3 py-2 text-text-muted hover:text-text-main transition-colors cursor-pointer"
                                >−</button>
                                <span className="px-4 py-2 text-text-main font-medium min-w-[40px] text-center">{quantity}</span>
                                <button
                            ...
                            <table className="w-full">
                                <tbody>
                                    {Object.entries(specs).map(([key, value], i) => (
                                        <tr key={key} className={i % 2 === 0 ? 'bg-card' : 'bg-primary'}>
                                            <td className="px-4 py-3 text-text-muted text-sm font-medium w-1/3 capitalize">
                                                {key.replace(/_/g, ' ')}
                                            </td>
                                            <td className="px-4 py-3 text-text-main text-sm">{String(value)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                            ...
                            {reviews.length > 0 ? reviews.map(r => (
                                <div key={r.id} className="bg-card rounded-xl border border-border p-4">
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <span className="text-text-main font-medium text-sm">{r.title}</span>
                                    </div>
                                    <p className="text-text-muted text-sm">{r.body}</p>
                                    <p className="text-text-muted text-xs mt-2">{r.created_at}</p>
                                </div>
                            )) : (
                                <p className="text-text-muted text-center py-8">ยังไม่มีรีวิวสำหรับสินค้านี้</p>
                            )}
                        </div>
                    )}
                </div>
            ...
            {related.length > 0 && (
                <section className="mt-8">
                    <h2 className="text-xl font-bold text-text-main mb-4">🔗 สินค้าที่เกี่ยวข้อง</h2>
