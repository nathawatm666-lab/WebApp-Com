...
export default function AdminDashboard() {
    ...
    return (
        <div>
            <h1 className="text-2xl font-bold text-text-main mb-6">📊 แดชบอร์ด</h1>
            ...
            <div className="bg-card rounded-xl border border-border p-6 mb-6">
                <h2 className="text-lg font-semibold text-text-main mb-4">📈 ยอดขาย (30 วัน)</h2>
            ...
            <div className="bg-card rounded-xl border border-border overflow-hidden">
                <div className="p-5 border-b border-border">
                    <h2 className="text-lg font-semibold text-text-main">🛒 คำสั่งซื้อล่าสุด</h2>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-border text-left">
                                <th className="px-5 py-3 text-xs text-text-muted font-medium">ออเดอร์</th>
                                <th className="px-5 py-3 text-xs text-text-muted font-medium">วันที่</th>
                                <th className="px-5 py-3 text-xs text-text-muted font-medium">สถานะ</th>
                                <th className="px-5 py-3 text-xs text-text-muted font-medium">การชำระ</th>
                                <th className="px-5 py-3 text-xs text-text-muted font-medium text-right">ยอดรวม</th>
                            </tr>
                        </thead>
                        <tbody>
                            {loading ? (
                                <tr><td colSpan={5} className="px-5 py-8 text-center text-text-muted">กำลังโหลด...</td></tr>
                                ...
                                return (
                                    <tr key={order.id} className="border-b border-border/50 hover:bg-card-hover transition-colors">
                                        ...
                                        <td className="px-5 py-3 text-sm text-text-muted">{order.created_at}</td>
                                        <td className="px-5 py-3">
                                            <span className={`text-xs px-2 py-1 rounded-lg font-medium ${status.color}`}>{status.text}</span>
                                        </td>
                                        <td className="px-5 py-3 text-sm text-text-muted capitalize">{order.payment_method}</td>
                                        <td className="px-5 py-3 text-sm text-text-main font-semibold text-right">{formatPrice(order.total)}</td>
                                    </tr>
                                );
                            })}
                        </tbody>
