    ...
    return (
        <div className="max-w-4xl mx-auto px-4 py-6">
            <nav className="flex items-center gap-2 text-sm text-gray-500 mb-6">
                ...
                <span>/</span>
                <span className="text-white">ประวัติคำสั่งซื้อ</span>
            </nav>

            <h1 className="text-2xl font-bold text-white mb-6">📦 ประวัติคำสั่งซื้อ</h1>

            {loading ? (
                <div className="space-y-3">
                    {[1, 2, 3].map(i => <div key={i} className="h-24 bg-dark-card rounded-xl animate-pulse" />)}
                </div>
            ) : orders.length === 0 ? (
                ...
                <div className="space-y-3">
                    {orders.map(order => {
                        ...
                        return (
                            <div key={order.id} className="bg-dark-card rounded-xl border border-dark-border p-5">
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                                    <div>
                                        <span className="text-accent font-mono font-bold">{order.order_number}</span>
                                        <span className="text-gray-500 text-sm ml-3">{order.created_at}</span>
                                    </div>
                                ...
                                <div className="space-y-1 mb-3">
                                    {order.items?.map((item, i) => (
                                        <div key={i} className="flex justify-between text-sm">
                                            ...
                                            <span className="text-gray-300">{formatPrice(item.subtotal)}</span>
                                        </div>
                                    ))}
                                </div>
