...
export default function HeroBanner() {
    ...
    return (
        <div className="relative border-y border-border bg-card overflow-hidden transition-colors duration-1000 my-8">
            <div className="grid grid-cols-1 lg:grid-cols-12 relative z-10">
                
                {/* Visual Accent Column */}
                <div className="hidden lg:flex col-span-1 border-r border-border flex-col justify-between items-center py-8">
                    ...
                    <div className="flex flex-col gap-2">
                        {banners.map((_, i) => (
                            <button
                                ...
                                className={`w-1 transition-all duration-500 ease-out cursor-pointer ${i === current ? 'bg-accent h-12' : 'bg-border h-4 hover:bg-text-muted'}`}
                            />
                        ))}
                    </div>
                </div>
                ...
                <div className="col-span-1 lg:col-span-7 px-6 py-16 md:py-24 md:px-12 flex flex-col justify-center">
                    ...
                    <h1 className="text-4xl md:text-5xl lg:text-7xl font-bold text-text-main mb-6 uppercase tracking-tighter leading-[0.9] animate-[fadeInUp_0.6s_cubic-bezier(0.16,1,0.3,1)_0.1s_both]">
                        {banner.title}
                    </h1>
                    
                    <p className="text-text-muted font-mono text-sm md:text-base mb-10 max-w-xl animate-[fadeInUp_0.6s_cubic-bezier(0.16,1,0.3,1)_0.2s_both] leading-relaxed">
                        // {banner.subtitle}
                    </p>
                    ...
                    <div className="animate-[fadeInUp_0.6s_cubic-bezier(0.16,1,0.3,1)_0.3s_both]">
                        <Link
                            to={banner.link}
                            className="inline-flex items-center gap-4 bg-transparent border border-accent hover:bg-accent text-text-main hover:text-white font-mono text-sm font-bold uppercase tracking-widest px-8 py-4 transition-all duration-300 group"
                        >
                            {banner.cta}
                ...
                <div className="col-span-1 lg:col-span-4 border-l border-border bg-card-hover relative overflow-hidden hidden md:block opacity-60">
                    ...
                    <div className="w-full h-full p-8 flex flex-col justify-end relative z-20">
                        ...
                        <div className="h-64 border border-border relative group">
                            {/* Abstract scanlines and blocks representing tech UI */}
                            <div className="absolute top-0 right-0 w-16 h-16 border-l border-b border-border"></div>
            ...
            <div className="lg:hidden absolute bottom-0 left-0 w-full h-1 flex">
                {banners.map((_, i) => (
                    <div key={i} className={`flex-1 transition-colors duration-500 ease-out ${i === current ? 'bg-accent' : 'bg-border'}`}></div>
                ))}
            </div>
        </div>
    );
